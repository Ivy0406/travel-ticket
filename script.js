...
function renderCards(ticketsToRender) {
  dom.searchResult.textContent = `本次搜尋共 ${ticketsToRender.length} 筆資料`;
...
function addNewCard() {
  ...
  const newCard = {
    ...
    imgUrl:
      inputs["ticket-img-url"].value || Default_Img,
    ...
    rate: Number(inputs["ticket-price"].value)
  };
  allTicketsData.push(newCard);
  renderCards(allTicketsData);
